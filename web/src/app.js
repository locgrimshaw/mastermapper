import { initLondonSift } from "./londonsift.js?v=ls2";
...
const MAP_OVERLAYS = [
  ...
  { key: "osm_industrial", group: "bfbuilt", label: "Industrial estates & business parks", color: "#5f3dc4",
  ...
  // London Plan designations from the GLA planning data map (migration 0084).
  { key: "gla_opportunity_area", group: "policy", label: "Opportunity Areas (London Plan)", color: "#9c36b5", dataset: "gla_opportunity_area", minZoom: 8, nameLabel: true },
  { key: "gla_sil", group: "policy", label: "Strategic Industrial Locations (London)", color: "#8b5a2b", dataset: "gla_sil", minZoom: 9 },
  { key: "gla_mol", group: "policy", label: "Metropolitan Open Land (London)", color: "#2b8a3e", dataset: "gla_mol", minZoom: 9 },
  ...
  // The same measure at OUTPUT AREA level — 188,884 areas of ~125 households,
  // 5.6x the detail of the neighbourhood layer and as fine as DfT publishes.
  // Its own registry entry rather than a third zoom band on the layer above,
  // because tiles and the bbox RPC use the same ov-<key>-* layer ids and
  // cannot share one key. Served from PMTiles for the reason PTAL is: the RPC
  // spends ~400 us per feature building JSON, so 188,884 polygons are
  // unservable per viewport however the query is tuned.
  { key: "connectivity_oa", group: "access",
    label: "Connectivity · Output Area detail", color: "#00b48c",
    // z12 floor, deliberately. tippecanoe's --coalesce is what keeps a tiled
    // surface small, and it merges only features with IDENTICAL attributes —
    // with 35 unique floats per area nothing coalesces, so a z10 tile over
    // London would carry ~10,000 areas x 35 properties. From z12 a tile holds
    // a few hundred, and the neighbourhood layer covers everything below.
    dataset: "connectivity_oa", minZoom: 12, conn: true,
    tiles: { file: "connectivity_oa.pmtiles", sourceLayer: "connectivity_oa",
             minzoom: 12 } },
...
const LAYER_INFO = {
  ...
  osm_industrial:     { about: "Employment land and low-rise sheds mapped in OpenStreetMap — industrial estates, warehouses, factories and works, plus <strong>business parks and trading estates</strong>: commercial land carrying the out-of-town signature of a large site with a large surface car park (Slough Trading Estate, Gloucester Business Park, Chester Business Park). Those sit here rather than under retail because that is what they are — single- and two-storey sheds on surface parking, exactly the \"underutilised … business sites\" NPPF policy L2(1)(b) names as attracting <strong>substantial weight</strong>. All of it is <strong>previously developed land</strong> under NPPF Annex B. Anything of three storeys or more is dropped: a multi-storey office is a building, not a shed. Nothing here is a registered site — this is the built form, so it finds the shed the register never mentions. The area floor rises as you zoom out; below z12 only sites over 2 ha are drawn. Area is measured from the mapped polygon, not from a title.", source: "OpenStreetMap contributors (ODbL 1.0), classified in pipeline/build_osm_lowvalue.py" },
  osm_retail:         { about: "Out-of-town retail formats — retail parks, superstores, DIY and trade sheds, car dealerships, garden centres and filling stations. <strong>Not high-street shops</strong>: OSM tags whole retail districts (Oxford Street, Mayfair) as <em>retail land</em> and every shop building as <em>retail</em>, so this layer demands evidence of shed-ness instead — an out-of-town format, a retail-park name, or a large low-rise footprint with its own surface car park beside it. Department stores are excluded outright and anything of three storeys or more is dropped. Business parks and trading estates are <em>not</em> here — OSM maps them as commercial land and they carry the same out-of-town signature, so they are classified under <em>Industrial estates &amp; business parks</em> instead. <strong>Previously developed land</strong>; NPPF L2(1)(b) names \"redeveloping underutilised retail sites\" as attracting substantial weight.", source: "OpenStreetMap contributors (ODbL 1.0)" },
  ...
  osm_leisure_lowdensity: { about: "Golf courses, driving ranges, marinas and tracks — large, low-value, lightly built land. Shown because it is a real land supply, but flagged <strong>NOT previously developed land</strong>: NPPF Annex B excludes parks, recreation grounds and undeveloped leisure land from the PDL definition unless permanent structures are involved, so these do not attract the L2 brownfield weighting and are a materially harder planning case than anything else in this group. Garden centres sit under retail instead, since their glasshouses, buildings and hardstanding do make them previously developed.", source: "OpenStreetMap contributors (ODbL 1.0)" },
...
async function ensureOverlayTiles(key, def) {
  ...
  if (!map.getLayer(fillId))
    map.addLayer({ id: fillId, type: "fill", source: srcId,
      ...
      paint: { "fill-color": def.conn ? connPaint()
                 : (def.cap && def.cap.color) || def.color,
...
function hoverContentForOverlay(def, p) {
  ...
  if (d === "power_substation") {
    ...
    kind = `Connectivity — ${connMetricLabel()}` +
      (d === "connectivity_lad" ? " (authority average)"
       : d === "connectivity_oa" ? " (output area)" : " (neighbourhood)");
  ...
  } else if (d === "gla_sil" || d === "gla_mol") {
    title = p.name || (d === "gla_sil" ? "Strategic Industrial Location" : "Metropolitan Open Land");
    kind = d === "gla_sil" ? "Strategic Industrial Location — London Plan E5"
                           : "Metropolitan Open Land — London Plan G3";
    rows = [row(p.borough, "borough"),
            row(p.hectares != null ? `${Number(p.hectares).toLocaleString()} ha` : null, "area"),
            row(d === "gla_sil" ? p.type : null, "designation")];
...
// NPPF (Aug 2026) policy S5(2)(c): at least 35 dwellings per hectare within
// reasonable walking distance of a well-connected station, "higher densities —
// of at least 45 dwellings per hectare — ... where the service frequency is at
// least twice that of the minimum required for a well-connected station".
// The well-connected minimum is 4 trains/hour overall or 2 per direction, so
...
// The statutory minimum density across the stations still in play, which is a
// per-station fact rather than a single number: policy S5(2)(c) raises the
// floor from 35 to 45 dph where service frequency is at least double the
// well-connected minimum. Shown so the user setting a density knows which
// floor they are above — and it is a floor to exceed, not a target.
function siftNppfMinHTML() {
  const rows = siftCountryRows();
  if (!rows.length) return "";
  let high = 0, base = 0;
  for (const r of rows) {
    const m = nppfMinDph(r.crs);
    if (!m) continue;
    if (m.doubleFreq) high++; else base++;
  }
  if (!high && !base) return "";
  return `<p class="hint sift-keep">NPPF minimum here: <strong>${NPPF_MIN_DPH} dph</strong> ` +
    `near a well-connected station, rising to <strong>${NPPF_MIN_DPH_HIGH} dph</strong> ` +
