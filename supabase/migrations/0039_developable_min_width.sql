-- 0039_developable_min_width.sql
-- Developable land came back with hair on it: long thin slivers left between a
-- constraint edge and the catchment edge (a road verge, a strip beside a railway
-- embankment). They pass the min_plot AREA test — a 4 m x 300 m ribbon is 0.3 ac
-- — but nothing can be built on them, and they inflate the dwelling capacity.
--
-- min_width_m adds a SHAPE test to the existing area test: a morphological
-- OPENING (erode by w/2, dilate back by w/2, then clip to the original). Any
-- part of the geometry that is nowhere at least w metres wide disappears;
-- anything wider survives with its shape essentially intact. Crucially it works
-- WITHIN a plot too, so a good plot that trails a thin tail loses only the tail
-- rather than being kept or dropped whole.
--
-- Two details that matter:
--   * Buffering happens in a local UTM zone chosen from the catchment centre,
--     not in 4326 (degrees are not metres) and not in 27700 (fine for GB, but
--     it distorts outside it). UTM is conformal, so a metre is a metre.
--   * The dilated result is intersected back with the input, so the closing
--     step can never push developable land into a constraint it was subtracted
--     from — the opening can only ever REMOVE area.
--
-- min_width_m defaults to 0 (no shape test) so existing callers are unchanged;
-- the frontend passes its own default.

-- Adding a parameter makes a new OVERLOAD, it does not replace the old one, and
-- a 7-argument call would then match both and fail as ambiguous. Drop the
-- previous signature first.
drop function if exists public.developable_land_near_station(double precision,
  double precision, double precision, double precision, text[], double precision,
  boolean);
...
language sql stable as $function$
  ...
  -- Local UTM zone for the metre-accurate buffering below (WGS84 north).
  ...
  -- Shape test: drop everything narrower than min_width_m, anywhere it occurs.
  dev_open as (
    select case
...
grant execute on function public.developable_land_near_station(double precision, double precision,
  double precision, double precision, text[], double precision, boolean, double precision)
