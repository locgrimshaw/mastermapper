-- 0017_subtract_water.sql
-- Add the new 'water' constraint kind (inland + tidal water bodies from OS
-- OpenMap Local: SurfaceWater_Area + TidalWater) to the developable-land erase
-- set, so lakes / reservoirs / estuaries are removed from the developable area
-- (previously they were left in). Water is a hard exclusion.
--
-- Only the default `subtract` array of the live RPC changes here.
-- rebuild_station_assessments (0015) calls the RPC with 3 positional args, so it
-- inherits this new default automatically on the next rebuild — no change needed
-- to that function.
