-- 0067_standoff_preclip.sql
--
-- Fix a severe performance regression in 0066. The standoff buffered each
-- constraint row's FULL geometry before clipping to the catchment:
--
--   st_intersection(st_buffer(st_transform(c.geom, utm), standoff), catchment)
--
-- transport rows are region-scale dissolved corridors (10,158 rows for GB), so
-- every station paid to buffer motorway networks miles outside its 800 m
-- circle. Measured: a 104-station prefix went from ~3 minutes pre-0066 to
-- blowing a 12-minute statement budget.
--
-- The cheap order is pre-clip -> buffer -> final clip: intersect the row with
-- the catchment EXPANDED by the standoff first (so geometry that could reach
-- the catchment after buffering is kept, and only a circle's worth of it),
-- buffer that small piece, then clip to the true catchment. The bbox prefilter
-- moves to the expanded ring for the same reason.

drop function if exists public.developable_land_near_station(
  double precision, double precision, double precision, double precision,
  text[], double precision, boolean, double precision, double precision, double precision);
...
LANGUAGE sql STABLE AS $function$
  ...
  standoff as (select greatest(coalesce(transport_standoff_m,0),
                               coalesce(built_land_standoff_m,0)) as m),
  rings as (
    select st_buffer((select g from centre), radius_m, 16)::geometry as catchment,
           st_buffer((select g from centre),
                     radius_m + (select m from standoff), 16)::geometry as catch_x,
    ...
    select case
             when c.kind = 'transport' and coalesce(transport_standoff_m, 0) > 0
               then st_intersection(
                      st_transform(st_buffer(st_transform(
                        st_intersection(st_makevalid(c.geom), r.catch_x),
             ...
             when c.kind = 'built_land' and coalesce(built_land_standoff_m, 0) > 0
               then st_intersection(
                      ...
                      r.catchment)
             else st_intersection(st_makevalid(c.geom), r.catchment)
           end as g
    from public.planning_constraints c cross join rings r
    where c.kind = any(subtract) and st_intersects(c.geom, r.catch_x)),
  blockers as (
...
comment on function public.developable_land_near_station(
  ...
  'Developable land in a station catchment. transport_standoff_m (default 10) widens road/rail footprints to the strip that cannot be built on beside an adopted highway; built_land_standoff_m does the same for settlement edges (default 0). Standoff kinds are pre-clipped to the standoff-expanded catchment before buffering (0067).';
