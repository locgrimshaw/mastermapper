...
NPPF (Aug 2026) Annex B defines a 'well-connected' station as one that BOTH:
  (a) lies in a top-80 Travel to Work Area (TTWA), ranked by total Gross Value
...
top 80, map every station to its TTWA by point-in-polygon, flag
`ttwa_in_top`, and set `well_connected = meets_frequency AND ttwa_in_top`.

The cut was 60 in the December 2025 draft and is 80 in the Framework as made.
Correcting it moves 86 stations up a tier (949 well-connected to 1,035), so
the number is load-bearing — hence the auditable CSV below.
...
OUTPUT: updates web/data/stations.geojson in place, adding per station:
  ttwa_code, ttwa_name, ttwa_gva_rank, ttwa_in_top, well_connected
Also writes data/raw/ttwa_top80.csv — the ranked, auditable list (the cutoff
is policy-sensitive, so it must be inspectable).
...
TOP_CSV = RAW / "ttwa_top80.csv"

GVA_YEAR = "2023"   # latest available; configurable
TOP_N = 80          # NPPF Annex B: top 80 TTWAs by GVA (top 60 in the draft)
...
def rank_top(gva: dict) -> tuple[list, dict]:
...
def main() -> int:
    print(f"Completing NPPF well-connected test (TTWA-GVA top-{TOP_N} limb)...")
    for p in (TTWA_GEOJSON, TTWA_GVA_XLSX, STATIONS):
        if not p.exists():
            print(f"  missing input: {p}")
            return 1

    # 1. GVA -> ranked England-touching top N.
    gva = load_gva()
    print(f"  GVA ({GVA_YEAR}): {len(gva)} TTWAs total")
    ranked, code_to_rank = rank_top(gva)
    n_elig = len(ranked)
    top_codes = {code for rank, code, _g in ranked if rank <= TOP_N}
    ...
    # 3. Write the auditable ranked list (+ a few either side of the cutoff).
    ...
    for feat in sj["features"]:
        ...
        if found_code:
            ...
            props["ttwa_in_top"] = bool(found_code in top_codes)
            if found_code in top_codes:
                in_top += 1
        else:
            props["ttwa_code"] = None
            props["ttwa_name"] = None
            props["ttwa_gva_rank"] = None
            props["ttwa_in_top"] = False
        # Shed the draft-era key so nothing downstream can read a top-60 flag
        # and believe it is the current test.
        props.pop("ttwa_in_top60", None)
        # well_connected = frequency limb AND ttwa limb
        mf = props.get("meets_frequency")
        if mf is None:
            no_freq_field += 1
        props["well_connected"] = bool(mf) and bool(props["ttwa_in_top"])
    ...
    print(f"  stations in a top-{TOP_N} TTWA: {in_top}")
    if no_freq_field:
        print(f"  WARNING: {no_freq_field} stations had no meets_frequency "
              f"field — run build_station_usage.py first.")
    print(f"  WELL-CONNECTED (freq AND top-{TOP_N} TTWA): {wellconn} stations")

    # Show the cutoff neighbourhood — the politically sensitive boundary.
    print(f"\n  --- TTWA GVA ranking around the top-{TOP_N} cutoff ---")
    for rank, code, g in ranked:
        if TOP_N - 4 <= rank <= TOP_N + 4:
